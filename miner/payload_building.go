...
func (w *worker) buildPayload(args *BuildPayloadArgs) (*Payload, error) {
	...
	empty, _, _, err := w.buildBlock(args.Parent, args.Timestamp, args.FeeRecipient, args.Random, args.Withdrawals, true, nil, common.Address{}, 0, 0)
	if err != nil {
		return nil, err
	}
	...
	go func() {
		...
		for {
			...
			case <-timer.C:
				start := time.Now()
				block, fees, _, err := w.buildBlock(args.Parent, args.Timestamp, args.FeeRecipient, args.Random, args.Withdrawals, false, nil, common.Address{}, 0, 0)
				if err == nil {
