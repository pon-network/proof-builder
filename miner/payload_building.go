...
func (w *worker) buildPayload(args *BuildPayloadArgs) (*Payload, error) {
	...
	empty, _, _, _, _, err := w.getSealingBlock(args.Parent, args.Timestamp, args.FeeRecipient, args.Random, args.Withdrawals, true, nil, nil, common.Address{}, nil, 0)
	if err != nil {
		return nil, err
	}
	...
	go func() {
		...
		for {
			...
			case <-timer.C:
				start := time.Now()
				block, fees, _, _, _, err := w.getSealingBlock(args.Parent, args.Timestamp, args.FeeRecipient, args.Random, args.Withdrawals, false, nil, nil, common.Address{}, nil, 0)
				if err == nil {
