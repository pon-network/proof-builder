...
var (
	...
	BuilderWalletPrivateKey = &cli.StringFlag{
		Name:     "builder.wallet_private_key",
		Usage:    "Builder key used for signing transactions",
		EnvVars:  []string{"BUILDER_WALLET_PRIVATE_KEY"},
		Category: flags.BuilderCategory,
	}
	BuilderExposedAccessPoint = &cli.StringFlag{
		Name:     "builder.public_accesspoint",
		Usage:    "Builder Access Point for web server",
		EnvVars:  []string{"BUILDER_EXPOSED_ACCESSPOINT"},
	...
	BuilderBeaconEndpoints = &cli.StringFlag{
		...
		Usage:    "Beacon endpoints to connect to for beacon chain data (comma separated)",
		EnvVars:  []string{"BUILDER_BEACON_ENDPOINTS"},
		Value:    "http://127.0.0.1:5052",
		Category: flags.BuilderCategory,
	}
	BuilderRelayEndpoint = &cli.StringFlag{
		Name:     "builder.relay_endpoint",
		Usage:    "Relay endpoint to connect to for remote relay data",
	...
	BuilderRPBS = &cli.StringFlag{
		...
		EnvVars:  []string{"BUILDER_RPBS"},
		Value:    "",
		Category: flags.BuilderCategory,
	}
	BuilderMetricsEnabled = &cli.BoolFlag{
		Name:     "builder.metrics",
		Usage:    "Enable PoN builder metrics collection and reporting",
		Category: flags.MetricsCategory,
	}
	BuilderMetricsReset = &cli.BoolFlag{
		Name:     "builder.metrics_reset",
		Usage:    "Resets (zeros) the builder metrics upon start of the node",
		Category: flags.MetricsCategory,
	}
	BuilderBundlesEnabled = &cli.BoolFlag{
		Name:     "builder.bundles",
		Usage:    "Enable PoN builder bundle service",
		Category: flags.BuilderCategory,
	}
	BuilderBundlesReset = &cli.BoolFlag{
		Name:     "builder.bundles_reset",
		Usage:    "Resets the builders knowledge of received bundles upon start of the node, and removes any unprocessed transactions from any added bundles",
		Category: flags.BuilderCategory,
	}
	BuilderBundlesMaxLifetime = &cli.IntFlag{
		Name:     "builder.bundles_max_lifetime",
		Usage:    "Maximum lifetime of a bundle in seconds (s)",
		Category: flags.BuilderCategory,
	}
	BuilderBundlesMaxFuture = &cli.IntFlag{
		Name:     "builder.bundles_max_future",
		Usage:    "Maximum future time of a bundle in seconds (s)",
		Category: flags.BuilderCategory,
	}
	BuilderSubmissionEndWindow = &cli.IntFlag{
		Name:     "builder.submission_end_window",
		Usage:    "Time in seconds (s) before the end of the bid period that the builder will finalize the last block bid submission",
		Category: flags.BuilderCategory,
	}
	BuilderEngineRateLimit = &cli.IntFlag{
		Name:     "builder.engine_rate_limit",
		Usage:    "Rate limit in milliseconds (ms) for engine requests to rebuild block for a given slot",
		Category: flags.BuilderCategory,
	}
...
// SetBuilderConfig applies pon-builder-related command line flags to the config.
func SetBuilderConfig(ctx *cli.Context, cfg *builder.Config) {
	cfg.Enabled = ctx.IsSet(BuilderEnabled.Name)
	if cfg.Enabled {

		if ctx.IsSet(BuilderRPBS.Name) {
			cfg.RPBS = ctx.String(BuilderRPBS.Name)
		} else {
			Fatalf("-%s: RPBS service is required for PoN Builder", BuilderRPBS.Name)
			return
		}

		if ctx.IsSet(BuilderSecretKey.Name) {
			cfg.BuilderSecretKey = ctx.String(BuilderSecretKey.Name)
		} else {
			Fatalf("-%s: builder BLS secret key is required for PoN Builder", BuilderSecretKey.Name)
			return
		}

		if ctx.IsSet(BuilderListenAddr.Name) {
			cfg.ListenAddr = ctx.String(BuilderListenAddr.Name)
		} else {
			Fatalf("-%s: listen address is required for PoN Builder", BuilderListenAddr.Name)
			return
		}

		if ctx.IsSet(BuilderBeaconEndpoints.Name) {
			endpoints := strings.Split(ctx.String(BuilderBeaconEndpoints.Name), ",")
			...
			if len(validEndpoints) == 0 {
				Fatalf("-%s: beacon endpoint is required for PoN Builder", BuilderBeaconEndpoints.Name)
				return
			}

			cfg.BeaconEndpoints = validEndpoints
		} else {
			Fatalf("-%s: beacon endpoint is required for PoN Builder", BuilderBeaconEndpoints.Name)
			return
		}

		if ctx.IsSet(BuilderRelayEndpoint.Name) {
			cfg.RelayEndpoint = ctx.String(BuilderRelayEndpoint.Name)
		} else {
			Fatalf("-%s: relay endpoint is required for PoN Builder", BuilderRelayEndpoint.Name)
			return
		}

		if ctx.IsSet(BuilderExposedAccessPoint.Name) {
			cfg.ExposedAccessPoint = ctx.String(BuilderExposedAccessPoint.Name)
		} else {
			Fatalf("-%s: exposed access point is required for PoN builder", BuilderExposedAccessPoint.Name)
			return
		}

		if ctx.IsSet(BuilderWalletPrivateKey.Name) {
			pk, err := crypto.HexToECDSA(strings.TrimPrefix(ctx.String(BuilderWalletPrivateKey.Name), "0x"))
			if err != nil {
				Fatalf("-%s: invalid builder wallet private private key %q", BuilderWalletPrivateKey.Name, pk)
				return
			}
			cfg.BuilderWalletPrivateKey = pk

			publicKey := pk.Public()

			walletAddress := crypto.PubkeyToAddress(*publicKey.(*ecdsa.PublicKey))

			cfg.BuilderWalletAddress = walletAddress
		} else {
			Fatalf("-%s: builder wallet private key is required for PoN Builder", BuilderWalletPrivateKey.Name)
			return
		}

		cfg.MetricsEnabled = ctx.Bool(BuilderMetricsEnabled.Name)

		if cfg.MetricsEnabled {
			if ctx.IsSet(DataDirFlag.Name) {
				cfg.DatabaseDir = MakeDataDir(ctx)
			} else {
				Fatalf("-%s: database dir is required for PoN Builder Metrics. Need to set geth --datadir flag", DataDirFlag.Name)
				return
			}
			if ctx.IsSet(BuilderMetricsReset.Name) {
				cfg.DatabaseReset = ctx.Bool(BuilderMetricsReset.Name)
			} else {
				cfg.DatabaseReset = false
			}
		}

		cfg.BundlesEnabled = ctx.Bool(BuilderBundlesEnabled.Name)

		if cfg.BundlesEnabled {
			if ctx.IsSet(DataDirFlag.Name) {
				cfg.DatabaseDir = MakeDataDir(ctx)
			} else {
				Fatalf("-%s: database dir is required for PoN Builder Bundle Service. Need to set geth --datadir flag", DataDirFlag.Name)
				return
			}

			if ctx.IsSet(BuilderBundlesReset.Name) {
				cfg.BundlesReset = ctx.Bool(BuilderBundlesReset.Name)
			} else {
				cfg.BundlesReset = false
			}

			if ctx.IsSet(BuilderBundlesMaxLifetime.Name) {
				cfg.BundlesMaxLifetime = ctx.Int(BuilderBundlesMaxLifetime.Name)
			} else {
				cfg.BundlesMaxLifetime = 3600 * 720 // default to 30 days in seconds
			}

			if ctx.IsSet(BuilderBundlesMaxFuture.Name) {
				cfg.BundlesMaxFuture = ctx.Int(BuilderBundlesMaxFuture.Name)
			} else {
				cfg.BundlesMaxFuture = 3600 * 720 // default to 30 days in seconds
			}
		}

		if ctx.IsSet(BuilderSubmissionEndWindow.Name) {
			
			// check that the value is a valid integer
			window, err := strconv.Atoi(ctx.String(BuilderSubmissionEndWindow.Name))
			if err != nil {
				Fatalf("-%s: invalid submission delay %q", BuilderSubmissionEndWindow.Name, window)
				return
			}

			// check that the delay is greater than or equal to 0
			if window < 0 {
				Fatalf("-%s: submission delay must be greater than or equal to 0", BuilderSubmissionEndWindow.Name)
				return
			}

			cfg.SubmissionEndWindow = window

		} else {
			cfg.SubmissionEndWindow = 2 // default to 2 seconds
		}

		if ctx.IsSet(BuilderEngineRateLimit.Name) {

			// check that the value is a valid integer
			rateLimit, err := strconv.Atoi(ctx.String(BuilderEngineRateLimit.Name))
			if err != nil {
				Fatalf("-%s: invalid engine rate limit %q", BuilderEngineRateLimit.Name, rateLimit)
				return
			}

			// check that the rate limit is greater than 0
			if rateLimit <= 0 {
				Fatalf("-%s: engine rate limit must be greater than 0", BuilderEngineRateLimit.Name)
				return
			}

			cfg.EngineRateLimit = rateLimit

		} else {

			cfg.EngineRateLimit = 500 // default to 1 block build every 500ms
...
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config, bpCfg *builder.Config) (ethapi.Backend, *eth.Ethereum) {
	...
	if bpCfg.Enabled {
		if err := builder.Register(stack, backend, bpCfg); err != nil {
			Fatalf("Failed to register the PoN builder service: %v", err)
		}
	}
