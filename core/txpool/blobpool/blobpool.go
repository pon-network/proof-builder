...
func (p *BlobPool) RemoveTx(hash common.Hash) bool {
	p.lock.Lock()
	defer p.lock.Unlock()

	// Retrieve the associated blob from the limbo. This also removes the blob
	// from the limbo store.
	tx, err := p.limbo.pull(hash)
	if err != nil {
		log.Error("Blobs unavailable, dropping reorged tx", "err", err)
		return false
	}
	...
	// Find the blobtxmetadata associated with the transaction
	var (
		addr = from
		id   = p.lookup[tx.Hash()]
	)
	...
	for i, tx := range txs {
		if tx.id == id {
			// Remove the transaction from the pool's index
			p.index[addr] = append(txs[:i], txs[i+1:]...)
			p.spent[addr] = new(uint256.Int).Sub(p.spent[addr], tx.costCap)
			p.stored -= uint64(tx.size)
			delete(p.lookup, tx.hash)
			...
			// Remove the transaction from the data store
			log.Warn("Evicting reorged blob transaction", "from", addr, "evicted", tx.nonce, "id", tx.id)
